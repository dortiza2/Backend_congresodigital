...
public interface IAuthTokenService
{
    ...
    (string token, DateTime exp) CreateToken(long id, string email, string[]? roles, int roleLevel, string? fullName = null);
}
...
public class AuthTokenService : IAuthTokenService
{
    ...
    public (string, DateTime) CreateToken(long id, string email, string[]? roles, int roleLevel, string? fullName = null)
    {
        var jwt = _cfg.GetSection("Jwt");
        var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(jwt["Key"]!));
        var cred = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);
